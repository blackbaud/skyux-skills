...
it('should close the error modal after an invalid file is uploaded', async () => {
  ...
  await harness.dropAvatarFile(new File([], 'test.txt', { type: 'text/plain' }));
...
For test setup patterns, component harness usage, overlay testing, mocking strategies, and available SKY UX testing utilities, read [Angular testing patterns for SKY UX](./angular-testing-patterns.md). It covers:
...
Never fix bugs without a test. See the [skyux-migration-debugger](/skills/migration%20debugger/index) skill for the full systematic debugging process.

## Testing Anti-Patterns

When adding mocks or test utilities, read [testing antipatterns](/skills/test%20driven%20development/testing%20antipatterns) to avoid common pitfalls:
