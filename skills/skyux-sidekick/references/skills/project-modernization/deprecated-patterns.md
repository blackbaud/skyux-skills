...
it('should show initials', async () => {
  ...
  const avatar = await loader.getHarness(SkyAvatarHarness.with({ dataSkyId: 'test-avatar' }));
...
const child = fixture.debugElement.query(By.directive(StubChildComponent)).componentInstance;
```
...
// ✅ Stable: harness
const loader = TestbedHarnessEnvironment.loader(fixture);
const inputBox = await loader.getHarness(SkyInputBoxHarness.with({ dataSkyId: 'my-input' }));
