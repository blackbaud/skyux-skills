...
> **Prerequisite:** The application MUST currently build and pass tests before using this skill. If the user reports build errors, test failures, or mentions fixing a migration, update, or upgrade, use the [migration-debugger](../migration-debugger/README.md) skill FIRST to stabilize the project. Return to this skill only after the human partner confirms the app is green.

**Use when ALL of these are true:**

1. The application builds successfully (`ng build` passes)
2. Existing tests pass (`ng test` passes)
3. The human partner has confirmed the project is in a stable state
4. The user wants to incrementally improve the codebase

**Specific triggers (only when the prerequisites above are met):**
...
- User says "fix my migration", "fix my update", or "fix my upgrade" — use [migration-debugger](../migration-debugger/README.md)
- User reports build errors, test failures, or compilation errors after an upgrade
- Application is in a broken state after running `ng update`, `skyux update`, or migration schematics
- The word "fix" appears alongside "migration", "update", or "upgrade"

For these scenarios, use the [migration-debugger](../migration-debugger/README.md) skill first.
...
See `available-schematics.md` for the complete reference.
...
See `deprecated-patterns.md` for the complete lookup table.
...
1. **Replace `Sky*Fixture` → `Sky*Harness`**
   ...
   - See the [skyux-migration-debugger](../migration-debugger/README.md) reference `angular-debugging.md` for the Fixture-to-Harness migration guide
...
5. **Replace `jasmine.createSpyObj` for SKY UX services → testing controllers**
   ...
   - See the [skyux-test-driven-development](../test-driven-development/README.md) reference `angular-testing-patterns.md` for the full list
...
- If the test fails, use the [skyux-migration-debugger](../migration-debugger/README.md) skill to investigate
...
2. **Use the [skyux-test-driven-development](../test-driven-development/README.md) skill**
...
3. **Test structure for each component:**
   ...
   async function setupTest(): Promise<{
     ...
     const harness = await loader.getHarness(SkyExampleHarness.with({ dataSkyId: 'test-example' }));
...
- [Complete list of SKY UX and Angular schematics with commands and verification steps](./available-schematics.md)
- [Lookup table of deprecated patterns and their modern replacements](./deprecated-patterns.md)

**Related skills:**

- **[skyux-migration-debugger](../migration-debugger/README.md)** — For debugging issues that arise during modernization
- **[skyux-test-driven-development](../test-driven-development/README.md)** — For Phase 4 test coverage work and harness-first testing
- **[skyux-verification-before-completion](../verification-before-completion.md)** — For verifying each modernization step
