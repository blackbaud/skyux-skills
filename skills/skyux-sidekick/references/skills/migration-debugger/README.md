...
**Use this skill FIRST when the user mentions any of these:**

- "migration" / "migrate" — fixing issues during or after a SKY UX or Angular migration
- "update" / "ng update" / "skyux update" — problems during or after running `ng update`, `skyux update`, or `npm update`
- "upgrade" — fixing errors after upgrading Angular or SKY UX major versions
- "fix my migration" / "fix skyux update" — any request to fix a migration or update process

These prompts indicate something is broken or the user needs systematic investigation. Route here, NOT to project-modernization.

**Also use for ANY technical issue:**
...
- Errors or failures during or after `ng update`, `skyux update`, or migration schematics
- Build failures after running `skyux update` or upgrading Angular/@skyux/ package versions
- TypeScript or template errors that appeared after a version upgrade
...
6. **Angular-Specific Investigation**
   ...
   **Harness Environment:**
   ...
   See `angular-debugging.md` in this directory for detailed guidance.
...
- [Trace bugs backward through the call stack to find the original trigger](./root-cause-tracing.md)
- [Add validation at multiple layers after finding the root cause](./defense-in-depth.md)
- [Replace arbitrary timeouts with condition polling; includes Angular `fakeAsync`/`tick` patterns](./condition-based-waiting.md)
- [Angular-specific debugging: change detection, DI, lifecycle hooks, zone.js, overlay testing](./angular-debugging.md)

**Related skills:**

- **[skyux-test-driven-development](../test-driven-development/README.md)** — For creating failing test case (Phase 4, Step 1)
- **[skyux-verification-before-completion](../verification-before-completion.md)** — Verify fix worked before claiming success
